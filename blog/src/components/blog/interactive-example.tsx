  ...
  return (
    <div className="not-prose my-6 rounded-xl border bg-card p-6 shadow-xs">
      ...
      <div className="mt-4 flex items-center gap-4">
        <button type="button" aria-label="Decrease count" className="grid size-9 place-items-center rounded-full border hover:bg-muted" onClick={() => setCount((value) => value - 1)}>
          <MinusIcon className="size-4" />
        </button>
        <output className="min-w-8 text-center text-2xl font-semibold" aria-live="polite">{count}</output>
        <button type="button" aria-label="Increase count" className="grid size-9 place-items-center rounded-full border hover:bg-muted" onClick={() => setCount((value) => value + 1)}>
